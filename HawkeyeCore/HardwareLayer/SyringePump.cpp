...
SyringePump::SyringePump (std::shared_ptr<CBOService> pCBOService)
	...
	reagentUsageUpdateCheckTimer_ = std::make_shared <boost::asio::deadline_timer> (_pCBOService->getInternalIosRef());
}
...
//      As of 2020-10-10 this file has not been needed.
	...
	auto onInitializeComplete = [this, callback](bool status) -> void
	{
		...
		_pCBOService->enqueueInternal (callback, status);
	};

	// Start off by reading the current SyringePump registers since the Direct command writes all of the registers.
	_pCBOService->getInternalIosRef().post ([this, callback, onInitializeComplete]() -> void {
...
void SyringePump::readAndCacheRegisters (std::function<void(bool)> callback) {
	...
	auto onReadAndCacheRegistersComplete = [this, callback](ControllerBoardOperation::CallbackData_t cbData) -> void {
		...
		if (cbData.status == ControllerBoardOperation::Success && cbData.bytesRead) {
			cbData.FromRx ((void*)&_registerCache, sizeof(_registerCache));
		...
		_pCBOService->enqueueInternal (callback, false);

	};

	auto tid = _pCBOService->CBO()->Query(SyringePumpReadRegistersOperation(), onReadAndCacheRegistersComplete);
	if (tid)
	{
...
void SyringePump::processAsync (std::function<void(bool)> callback, OperationSequence sequence) {
	...
	switch (sequence) {
			...
			auto onSetConfigurationComplete = [this, callback](bool status) -> void {
				...
				if (status) {
					...
					if (numConfigCommandsProcessed_ >= configCommands_.size()) {
						...
						_pCBOService->enqueueExternal (callback, status);
						return;
					}
		...
		case OperationSequence::SendDirectCommand:
		{
			...
			auto tid = _pCBOService->CBO()->Execute(SyringePumpSendDirectOperation(sendDirectCommand_, _registerCache), SyringeSMTimeoutMs, onSendDirectComplete);
			if (tid)
			{
		...
		case OperationSequence::Initialize:
		{
			...
			auto tid = _pCBOService->CBO()->Execute(SyringePumpInitializeOperation(), SyringeSMTimeoutMs, onInitializeComplete);
			if (tid)
			{
...
void SyringePump::setNextProcessSequence (std::function<void(bool)> callback, OperationSequence nextSequence) {
	...
	_pCBOService->getInternalIosRef().post (std::bind (&SyringePump::processAsync, this, callback, nextSequence));
}
...
void SyringePump::checkReagentUsage (boost::system::error_code ec) {
	...
	if (!_pCBOService->CBO()->Query (SyringePumpReadReagentErrorOperation(), onCheckReagentUsageUpdateComplete)) {
...
void SyringePump::setPosition (std::function<void(bool)> callback, uint32_t target_volume_uL, uint32_t speed) {
	...
	if (speed > MAX_SYRINGEPUMP_SPEED) {
		Logger::L().Log (MODULENAME, severity_level::warning, 
			"speed " + std::to_string(speed) + " > " + std::to_string(MAX_SYRINGEPUMP_SPEED) + " using " + std::to_string(MAX_SYRINGEPUMP_SPEED) + "ul/sec");
		speed = MAX_SYRINGEPUMP_SPEED;
	}

	int32_t volume_being_moved = std::abs((int32_t)target_volume_uL - (int32_t)_cur_volume_uL);
	...
	Logger::L().Log (MODULENAME, severity_level::debug3, "setPosition: current volume in syringe: " + std::to_string(_cur_volume_uL) + "uL");
...
//TODO: check for Vi-Cell.
	...
	auto tid = _pCBOService->CBO()->Execute(SyringePumpSetPositionOperation(target_volume_uL, speed), SyringeSMTimeoutMs, onSetPositionComplete);
	if (tid)
	{
...
bool SyringePump::getValve (SyringePumpPort& port) {
	...
	port = SyringePumpPort::FromPhysicalPort (_curPhysicalPort);
	Logger::L().Log (MODULENAME, severity_level::debug3, "getValve: " + port.ToString() + " valve");
...
void SyringePump::setValve (std::function<void(bool)> callback, SyringePumpPort port, SyringePumpDirection direction) {
	...
	if (physicalPort == _curPhysicalPort) {
	...
	auto tid = _pCBOService->CBO()->Execute(SyringePumpSetValveOperation(physicalPort, direction), SyringeSMTimeoutMs, onSetValveComplete);
	if (tid)
	{
...
bool SyringePump::getPhysicalValve (PhysicalPort& valveNum) {
	...
	Logger::L().Log (MODULENAME, severity_level::debug3, "getPhysicalValve: " + std::to_string(_curPhysicalPort));
...
void SyringePump::setPhysicalValve (std::function<void(bool)> callback, char physicalPortChar, SyringePumpDirection direction) {
	...
	Logger::L ().Log (MODULENAME, severity_level::debug1, "setPhysicalValve: set " + std::string(1, physicalPortChar) + " valve " + direction.ToString ());
	...
	auto onSetValveComplete = [this, callback, physicalPort](ControllerBoardOperation::CallbackData_t cbData) -> void {
		...
		bool status = false;
		if (cbData.status == ControllerBoardOperation::Success) {
			_curPhysicalPort = static_cast<PhysicalPort_t>(physicalPort);
			status = true;
		} else {
			Logger::L().Log (MODULENAME, severity_level::error,
...
bool SyringePump::getPosition (uint32_t & pos) {
	...
	Logger::L().Log (MODULENAME, severity_level::debug3, "getPosition: " + std::to_string(_cur_volume_uL));
...
void SyringePump::readVersion (std::function<void(bool)> callback) {
	...
	auto onReadVersionComplete = [this, callback](ControllerBoardOperation::CallbackData_t cbData) -> void {
		...
		{
			cbData.FromRx ((void*)&_registerCache, sizeof (_registerCache));
			_version = std::string(_registerCache.PumpFirmwareVersion);
			status = true;
			Logger::L().Log (MODULENAME, severity_level::debug1, "readVersion <exit, version: " + _version + ">");
		} else {
			Logger::L().Log (MODULENAME, severity_level::error,
	...
	auto tid = _pCBOService->CBO()->Query(SyringePumpReadRegistersOperation(), onReadVersionComplete);
	if (tid)
	{
