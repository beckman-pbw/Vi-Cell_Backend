...
for logfile in common_func.getLogfileList():
    with open(logfile, "r") as fin, open(common_func.getDataFilename(),"a") as fdata, open(common_func.getLinesFilename(),"a") as flines:
        ...
        if headerWritten is False:
            ...
            fdata.write(',')
            headerWritten = True
            lines, lengths, temp_lines = common_func.getBlockLines(searchlines, "setValve: <enter>", [], False)
            for i, line in enumerate(lines):
                tempstring = line
                left,sep,right = tempstring.partition('<enter> set')
                if sep:
                    tempstring = right
                    left,sep,right = tempstring.partition('valve')
                    if sep:
                        fdata.write(left + ',')                
        ...
        for i, line in enumerate(lines):
            ...
            if "setValve: <enter>" in line:
                startTimeStr = line
                flines.write(line)

            if "setValve: <exit>" in line:
                endTimeStr = line
                flines.write(line)
